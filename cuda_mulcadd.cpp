...
__global__ void apply_threshold_column_kernel(int *matrix, int *col_sums, int threshold, int *result_matrix) {
    ...
    if (col_idx < P) {
        if (col_sums[col_idx] < threshold) {
            for (int row_idx = 0; row_idx < M; row_idx++) {
                result_matrix[row_idx * P + col_idx] = 0;
            }
        } else {
            for (int row_idx = 0; row_idx < M; row_idx++) {
                result_matrix[row_idx * P + col_idx] = 1;
            }
        }
    }
}
...
int main() {
    ...
    int *d_matrix_a, *d_matrix_b, *d_result_matrix, *d_col_sums, *d_modified_matrix;
    ...
    cudaMalloc((void **)&d_modified_matrix, size_result);
    ...
    apply_threshold_column_kernel<<<gridSize_col, blockSize_col>>>(d_result_matrix, d_col_sums, threshold_value, d_modified_matrix);

    cudaMemcpy(result_matrix, d_modified_matrix, size_result, cudaMemcpyDeviceToHost);
