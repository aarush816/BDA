...
__global__ void apply_threshold_kernel(int *matrix, int *row_sums, int threshold, int *result_matrix) {
    int row_idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (row_idx < M) {
        // Check row sum against threshold and modify result_matrix accordingly
        result_matrix[row_idx] = (row_sums[row_idx] < threshold) ? 0 : 1;
    }
}
...
int main() {
    ...
    int size_rowsum = M * sizeof(int);

    // Allocate memory on device
    cudaMalloc((void **)&d_matrix_a, size);
    cudaMalloc((void **)&d_matrix_b, size);
    cudaMalloc((void **)&d_result_matrix, size);
    cudaMalloc((void **)&d_sum, size_rowsum);
    cudaMalloc((void **)&d_modified_matrix, M * sizeof(int));  // For thresholded result
    ...
    int thresholded_result[M];
    cudaMemcpy(thresholded_result, d_modified_matrix, M * sizeof(int), cudaMemcpyDeviceToHost);

    printf("\nThresholded Matrix (after row sum thresholding):\n");
    for (int i = 0; i < M; i++) {
        printf("%d ", thresholded_result[i]);
    }
    printf("\n");
